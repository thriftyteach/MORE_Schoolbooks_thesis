...
def filter_text_less(text, proper_nouns, stop_words_base):
    text = preprocess_text(text)
    doc = nlp(' '.join(text))  # Create spaCy doc from tokens
    words = [
        token.lemma_ for token in doc
        if token.lemma_ not in stop_words_base
        and token.lemma_ not in proper_nouns
        and len(token.lemma_) > 2  # Minimum word length
    ]
...
def process_text_files(directory, book_ranges, proper_nouns, stop_words_base):
    word_counts = Counter()
        ...
        for unit in units:
            ...
            if os.path.exists(filepath):
                with open(filepath, 'r', encoding='utf-8') as file:
                    ...
                    tokens = filter_text(text, proper_nouns, stop_words_base)
